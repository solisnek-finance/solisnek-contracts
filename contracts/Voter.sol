...
contract Voter is IVoter, Initializable {
    ...
    event GaugeCreated(address indexed gauge, address creator, address feeDistributer, address indexed pool);
    ...
    modifier onlyNewEpoch(uint256 _tokenId) {
        ...
        require(block.timestamp < IMinter(minter).active_period() + 1 weeks, "UPDATE_PERIOD");
        _;
    }
    ...
    function _reset(uint256 _tokenId) internal {
        ...
        for (uint256 i = 0; i < _poolVoteCnt; ++i) {
            ...
            if (_votes != 0) {
                ...
                if (_votes > 0) {
                    IFeeDistributor(feeDistributers[gauges[_pool]])._withdraw(uint256(_votes), _tokenId);
                    _totalWeight += _votes;
                } else {
                    _totalWeight -= _votes;
                }
    ...
    function _vote(uint256 _tokenId, address[] memory _poolVote, uint256[] memory _weights) internal {
        ...
        for (uint256 i = 0; i < _poolCnt; ++i) {
            ...
            if (isGauge[_gauge]) {
                ...
                IFeeDistributor(feeDistributers[_gauge])._deposit(uint256(_poolWeight), _tokenId);
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        if (msg.sender != governor) {
            // gov can create for any pool, even non-Snek pairs
            require(isPair, "!_pool");
            require(isWhitelisted[tokenA] && isWhitelisted[tokenB], "!whitelisted");
        }

        address _feeDistributer = IFeeDistributorFactory(feeDistributorFactory).createFeeDistributor(_pool);
        // return address(0);
        address _gauge = IGaugeFactory(gaugefactory).createGauge(_pool, _feeDistributer, _ve, isPair, allowedRewards);
    ...
    function whitelistGaugeRewards(address[] calldata _gauges, address[] calldata _rewards) external {
        require(msg.sender == governor);
    ...
    function removeGaugeRewards(address[] calldata _gauges, address[] calldata _rewards) external {
        require(msg.sender == governor);
