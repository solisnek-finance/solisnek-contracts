...
contract RewardsDistributor is IRewardsDistributor, Initializable {
    ...
    function initialize(address _voting_escrow, address _depositor) external initializer {
    ...
    function _checkpoint_token() internal {
        ...
        uint256 next_week = 0;
        uint256 weeks_to_catchup = ((block.timestamp / WEEK) * WEEK - this_week) / WEEK;
    ...
    function _claim(uint256 _tokenId, address ve, uint256 _last_token_time) internal returns (uint256) {
        ...
        for (uint256 i = 0; i < 50; i++) {
            ...
            uint256 balance_of = IVotingEscrow(ve).balanceOfNFTAt(_tokenId, week_cursor);
    ...
    function claim(uint256 _tokenId) public returns (uint256) {
        ...
        require(ve.isApprovedOrOwner(msg.sender, _tokenId) || ve.ownerOf(_tokenId) == address(0), "tokenId auth");
